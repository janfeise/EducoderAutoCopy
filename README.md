...
- 双账号登录与课程导航
- 精准定位未完成的实验并进入详情与关卡
- 从来源账号提取代码（Monaco/CodeMirror、剪贴板、DOM兜底）
- 将代码写入目标账号并自动提交测评
- 失败关卡自动记录并继续下一关
- 选择题自动识别并填写
- 可配置的关卡间等待与控制台倒计时输出
...
```
3) 配置环境变量（推荐）
- 在项目根目录创建 `.env`（已在 `.gitignore` 中忽略，不会入仓库）：
```
EDUCODER_USERNAME=你的未完成账号
EDUCODER_PASSWORD=对应密码
EDUCODER_COMPLETE_USERNAME=你的已完成账号
EDUCODER_COMPLETE_PASSWORD=对应密码
EDUCODER_COURSE_NAME=机器学习
EDUCODER_LOGIN_URL=https://www.educoder.net/
...
TIMEOUT_CLICK_TIMEOUT=8000
TIMEOUT_LEVEL_WAIT=600000
```
4) 运行脚本
```
...
## 配置说明
- 推荐通过环境变量或 `.env` 配置，避免敏感信息进入仓库。
- 如需本地文件配置，可复制 `autocopy/config.example.json` 为 `autocopy/config.json`（注意：该文件已在 `.gitignore` 中忽略，不会提交）。
- 配置优先级：环境变量/.env > 本地 `config.json` > 默认值。
- 关键字段：
...
- 敏感文件：`autocopy/config.json` 与 `.env` 已被忽略，勿提交。
...
- 等待时间太短或太长：
  - 调整 `TIMEOUT_LEVEL_WAIT` 或 `timeout.levelWait`，脚本会显示倒计时。
